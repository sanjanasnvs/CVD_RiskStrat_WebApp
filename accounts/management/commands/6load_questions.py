...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        print("🔄 Loading files...")
        # Extract used column names from sample, stripping the prefix
        used_columns = pd.read_csv(sample_file, header=None)[0].str.replace('category_Sociodemographics_ts_', '', regex=False).tolist()
        # Strip Excel column name whitespace
        mapping_df.columns = mapping_df.columns.str.strip()
        # Filter only rows in the mapping that match the sample
        ...
        # Filter only questions that exist in the sample CSV
